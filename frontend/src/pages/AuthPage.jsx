        ...
        {PASSWORD_RULES.map(rule => {
          ...
          return (
            <div key={rule.id} style={{display:'flex',alignItems:'center',gap:'8px',padding:'5px 0',transition:'all .2s'}}>
              ...
              <span style={{fontSize:'.82rem',color:passed?'#065F46':'#9CA3AF',fontWeight:passed?600:400,transition:'color .3s',textDecoration:passed?'none':'none',fontFamily:"'DM Sans',sans-serif"}}>{rule.label}</span>
            </div>
          )
        })}
      </div>
...
export default function AuthPage({ setupMode = false, userId = null } = {}) {
  ...
  async function doSignup() {
    ...
    if (!setupMode&&!isPasswordValid(form.password)) {
        ...
        <div style={{width:'100%',maxWidth:'420px'}}>
          ...
          {screen==='account' && !setupMode && (
            <div className="auth-step" style={{background:'#fff',borderRadius:'24px',padding:'40px 36px',boxShadow:'0 8px 40px rgba(0,0,0,.08)',border:'1px solid #F1F5F9'}}>
              ...
              <div style={{position:'relative'}}>
                <input style={{...INP,paddingRight:'44px',marginBottom: form.password ? '8px' : '14px',borderColor: form.password ? (isPasswordValid(form.password) ? '#1DB954' : '#E2E8F0') : '#E2E8F0'}} className="auth-inp" type={showPwd?'text':'password'} placeholder="Create a strong password" value={form.password} onChange={e=>up('password',e.target.value)}/>
                <button type="button" onClick={()=>setShowPwd(!showPwd)} style={{position:'absolute',right:'12px',top:'13px',background:'none',border:'none',cursor:'pointer',color:'#94A3B8',padding:'4px',display:'flex',alignItems:'center'}} title={showPwd?'Hide password':'Show password'}>
                  {showPwd ? <EyeOff size={18}/> : <Eye size={18}/>}
                </button>
              </div>
              ...
              <div style={{background:'#F0FDF4',border:'1px solid #A7F3D0',borderRadius:'10px',padding:'12px 14px',marginBottom:'18px',fontSize:'.84rem',color:'#065F46',display:'flex',gap:'8px',alignItems:'flex-start'}}>
                <ShieldCheck size={14} style={{flexShrink:0,marginTop:'1px'}}/> Your data is encrypted and private. We never share it with anyone.
              </div>
