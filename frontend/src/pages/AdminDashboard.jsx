...
export default function AdminDashboard({ profile }) {
  ...
  async function handleApproveProof(proofId, driverJobId, driverId, driverPayout) {
    ...
    toast.success(`✅ Approved + ₹${amount} added`); fetchProofs()
  }
  ...
  async function handleProcessPayout(payoutId) {
    ...
    if (error) return toast.error(error.message)
    toast.success('Payout marked as paid 💸'); fetchPayouts()
  }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F5F5F5', color:'#111', fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ padding:'16px', maxWidth:'900px', margin:'0 auto' }}>
        ...
        {tab==='campaigns' && <>
          {campaigns.length===0
            ? <div style={{ textAlign:'center', padding:'48px', color:'#bbb' }}><div style={{ fontSize:'3rem', marginBottom:'10px' }}>📢</div><div>No campaigns yet</div></div>
            : campaigns.map(c=>(
              <div key={c.id} style={card}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'12px', marginBottom:'14px', flexWrap:'wrap' }}>
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:'0.82rem', color:'#888', marginBottom:'8px' }}>📞 {c.users?.phone} · ₹{c.plans?.price?.toLocaleString()}/mo</div>
                    <span style={badge(c.status)}>{c.status}</span>
                  </div>
                ...
                <div style={{ borderTop:'1px solid #F0F0F0', paddingTop:'14px' }}>
                  {c.status==='pending' && (
                    <div style={{ display:'flex', alignItems:'center', gap:'12px', flexWrap:'wrap' }}>
                      <span style={{ fontSize:'0.82rem', color:'#888' }}>⏳ Waiting for payment</span>
                      <button style={btn('#FFBF00','#111')} onClick={()=>handleActivateCampaign(c.id)}>✅ Mark as Paid</button>
                    </div>
                  )}
                  {(c.status==='paid'||c.status==='active') && <>
                    ...
                    <div style={{ marginTop:'12px' }}>
                      {assigningJob===c.id
                        ? <div style={{ background:'#FFF8E6', border:'1px solid #FFE08A', borderRadius:'10px', padding:'12px', display:'flex', gap:'8px', alignItems:'center', flexWrap:'wrap' }}>
                        ...
                        : <button style={btn('#F5F5F5','#555')} onClick={()=>setAssigningJob(c.id)}><Plus size={13}/> Assign Driver</button>
                      }
                    </div>
                  </>}
                </div>
